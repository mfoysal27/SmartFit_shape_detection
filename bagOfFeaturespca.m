%bagOfFeaturespca Create bag of visual features.
%   bag = bagOfFeaturespca(imds) returns a bag of visual features. imds is
...
%   algorithm.
%
%   bag = bagOfFeaturespca(imds,'CustomExtractor',extractorFcn) returns a
...
%
%     <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'bagOfFeaturespcaCustomFeatures')">Learn more about writing a custom feature extractor.</a>
%
%     <a href="matlab:bagOfFeaturespca.createExtractorTemplate">Open a custom feature extractor template.</a>
%
%   bag = bagOfFeaturespca(...,Name,Value) specifies additional
...
%   
%   - bagOfFeaturespca supports parallel computing using multiple MATLAB
...
%   bag = bagOfFeaturespca(trainingSet); % bag creation can take a few minutes
%
...
%   extractor = @examplebagOfFeaturespcaExtractor;
%   bag = bagOfFeaturespca(imds,'CustomExtractor',extractor) 
%
%   % <a href="matlab:edit('examplebagOfFeaturespcaExtractor')">Open examplebagOfFeaturespcaExtractor for more details.</a>
%
...
classdef bagOfFeaturespca < vision.internal.EnforceScalarHandle & matlab.mixin.CustomDisplay
    ...
    methods (Access = public)
        ...
        %------------------------------------------------------------------
        % Constructor
        function this = bagOfFeaturespca(varargin)

            if nargin == 0
                
                % Just return empty object with all default configuration
                % settings.
                d = bagOfFeaturespca.getDefaultSettings;
                this = this.setParams(d);
                this.VocabularySize = 0; % this is actually an empty bag
                
            else            
                % parse the inputs
                [imgSets, params] = bagOfFeaturespca.parseInputs(varargin{:});
                ...
                printer.printMessage('vision:bagOfFeaturespca:vocabCreation', ...
                    this.VocabularySize);                
                ...
                printer.printMessage('vision:bagOfFeaturespca:finishedBoF').linebreak;                
            end
        ...
        function [featureVector, varargout] = encode(this, in, varargin)
            ...
            %  be grayscale or truecolor. bag is the bagOfFeaturespca object.
            ...
            %  bag = bagOfFeaturespca(trainingSet);
            % 
            ...
            if nargout == 2 && ~this.ExtractorOutputsLocations
                error(message('vision:bagOfFeaturespca:customNoLocations',...
                    func2str(this.CustomExtractor)));                
            end
                                    
            params = bagOfFeaturespca.parseEncodeInputs(in, varargin{:});
            ...
            if ~isnumeric(in)        
                ...
                printer.linebreak;
                printer.printMessage('vision:bagOfFeaturespca:encodeTitle');     
                printer.print('--------------------------------------\n');
                
                bagOfFeaturespca.printImageSetDescription(printer, in);

                if bagOfFeaturespca.isImageSet(in)
                    numImages = sum([in.Count]);
                    
                    featureVector = bagOfFeaturespca.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput);
                    ...
                    if numVarargout == 1
                        for i = 1:numel(in)
                            ...
                            printer.printMessageNoReturn('vision:bagOfFeaturespca:encodeStart', count, i);
                            ...
                            printer.printMessage('vision:bagOfFeaturespca:encodeDone');
                        end
                    ...
                    printer.linebreak;
                    printer.printMessage('vision:bagOfFeaturespca:encodeFinished').linebreak;
                    
                else
                    % input is a imageDatastore
                    printer.printMessageNoReturn('vision:bagOfFeaturespca:encodeStartDS', numel(in.Files));     
    ...
    methods (Hidden, Access = protected)
        ...
        function trimmedDescriptors = trimDescriptors(this, ...
            ...
            printer.printMessage('vision:bagOfFeaturespca:trimDescriptors', ...
            ...
            for i=1:numSets
                ...
                trimmedDescriptors(insertIdx,:) = pca(setDescriptors);
        ...
        function [descriptors, scores] = extractDescriptorsFromImageSet(this, imgSets, params)
            ...
            for categoryIndex=1:numSets
                ...
                printer.printMessageNoReturn('vision:bagOfFeaturespca:extractingFeatures',imgSets(categoryIndex).Count, categoryIndex);
                
                [descriptors{categoryIndex}, ...
                    scores{categoryIndex}] = this.extractDescriptorsFromSet(imgSets(categoryIndex), params);
                
                printer.printMessage('vision:bagOfFeaturespca:extractingFeaturesDone',...
        ...
        function [groupedDescriptors, groupedScores] = extractDescriptorsFromImageDS(this, imds, params)
            ...
            if isempty(imds.Labels)        
                ...
                elseif any(undefined)
                    ...
                    error(message('vision:bagOfFeaturespca:labelsUndefined'))
            ...
            printer.printMessageNoReturn('vision:bagOfFeaturespca:extractingFeaturesDS', numImages);
            ...
            printer.printMessage(...
                'vision:bagOfFeaturespca:extractingFeaturesDone', numFeatures);
            printer.linebreak;
            
            numCategories      = bagOfFeaturespca.getNumCategories(imds);
        ...
        function [descriptors, scores] = extractDescriptors(this, imgSets, params, printer)
            ...
            printer.linebreak;
            printer.printMessage('vision:bagOfFeaturespca:createBagTitle');
            printer.print('-------------------------\n');
            
            bagOfFeaturespca.printImageSetDescription(printer,imgSets);
             
            this.printPointSelectionInfo(printer);
            
            if bagOfFeaturespca.isImageSet(imgSets)
        ...
        function [features, varargout] = encodeScalarImageSet(this, imgSet, params)
            ...
            features = bagOfFeaturespca.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput); 
            words    = bagOfFeaturespca.allocateVisualWords(numImages);
        ...
        function clusterCenters = createVocabulary(this, descriptors, varargin)
            ...
            params = bagOfFeaturespca.parseCreateVocabularyInputs(varargin{:});
            ...
            if K == 0
                error(message('vision:bagOfFeaturespca:zeroVocabSize'))
            end
            
            if K < this.VocabularySize
                warning(message('vision:bagOfFeaturespca:reducingVocabSize', ...
        ...
        function points = createSURFPointsGrid(this, grayImage)            
            ...
            if isempty(rangeX) || isempty(rangeY)
                ...
                warning(message('vision:bagOfFeaturespca:noGridPoints'));
                points = pca(SURFPoints(zeros(0,2)));
            else
        ...
        function [features, featureMetrics, varargout] = invokeCustomExtractor(this, img, extractor)
            ...
            featureLength = bagOfFeaturespca.checkCustomExtractorSizes(features, featureMetrics, varargout{:});
            
            % Error if the feature length changes from the one we have
            % cached.
            if this.CustomFeatureLength ~= featureLength
                error(message('vision:bagOfFeaturespca:customInvalidFeatureLength', ...
        ...
        function printPointSelectionInfo(this, printer)
            ...
            if this.UsingCustomExtractor                                   
                ...
                printer.printMessage('vision:bagOfFeaturespca:customExtractionInfo', str);
            else
                printer.printMessage('vision:bagOfFeaturespca:extractionInfo1', this.PointSelection);
                printer.printMessage('vision:bagOfFeaturespca:extractionInfo2');
                
                if strcmpi(this.PointSelection,'Grid')
                    gs = deblank(sprintf('%d ', this.GridStep));
                    bw = deblank(sprintf('%d ', this.BlockWidth));
                    
                    printer.printMessage('vision:bagOfFeaturespca:gridInfo',gs,bw);
                else
                    cmd = 'helpview(fullfile(docroot,''toolbox'',''vision'',''vision.map''),''bagOfFeaturespcaSURFInfo'')';
                    cmdstr = printer.makeHyperlink('detectSURFFeatures',cmd);
                    
                    printer.printMessage('vision:bagOfFeaturespca:detectorInfo',...
                        cmdstr);
                end                           
            end
            printer.linebreak;
        ...
        function printFeatureBalancingMessage(this, printer, numFeatures, setIdx, numToKeep)
            ...
            if ~(strcmpi(this.PointSelection, 'Grid') && setsHaveSameNumFeatures)
                printer.printMessage('vision:bagOfFeaturespca:balanceNumFeatures');
                printer.printMessage('vision:bagOfFeaturespca:minNumFeatures',...
                    setIdx, numToKeep);
                printer.printMessage('vision:bagOfFeaturespca:minNumFeaturesInOthers',...
                    numToKeep).linebreak;
            end
    ...
    methods (Hidden, Static, Access = protected)
        ...
        function supportsLocation = checkCustomExtractor(func)
            ...
            if ~isVarargin && numIn ~= 1                                     
                error(message('vision:bagOfFeaturespca:customInvalidNargin'));               
            end
            
            if ~isVarargout && numOut < 2
                error(message('vision:bagOfFeaturespca:customInvalidNargout'));
            end
        ...
        function checkCustomExtractorOnLoad(func)
            ...
            try                 
                ...
                str = func2str(func);
                warning(message('vision:bagOfFeaturespca:customExtractorMissingOnLoad',...
                    str,str));
            end
        end
        ...
        function [tf, ps] = checkAndAssignPointSelection(pointSelection)
            ...
            ps = validatestring(pointSelection,...
                bagOfFeaturespca.ValidPointSelectionOptions, mfilename, 'PointSelection');
        ...
        function featureLength = validateCustomExtractor(imgSet, extractor) 
            ...
            if bagOfFeaturespca.isImageSet(imgSet)
            ...
            featureLength = bagOfFeaturespca.checkCustomExtractorSizes(features, metrics);
        ...
        function featureLength = checkCustomExtractorSizes(features, featureMetrics, featureLocations)
            ...
            if ~ismatrix(features)
                error(message('vision:bagOfFeaturespca:customInvalidFeatures'));
            end
            
            if ~iscolumn(featureMetrics)
                error(message('vision:bagOfFeaturespca:customInvalidMetrics'));
            end
            
            % features and metrics must be real floats (non-sparse).
            if ~isreal(features) || isinteger(features) || issparse(features)
                error(message('vision:bagOfFeaturespca:customFeaturesMustBeRealFloats'));
            end
            
            if ~isreal(featureMetrics) || isinteger(featureMetrics) || issparse(featureMetrics)
                error(message('vision:bagOfFeaturespca:customMetricsMustBeRealFloats'));
            end
            ...
            if featureLength == 0
                error(message('vision:bagOfFeaturespca:customZeroFeatureLength'));
            end
            
            % number of features and number of metrics must match
            if numFeatures ~= numMetrics
                error(message('vision:bagOfFeaturespca:customNumFeaturesNotEqNumMetrics'));
            end
            
            if nargin > 2
                if ~ismatrix(featureLocations)
                    error(message('vision:bagOfFeaturespca:customInvalidLocations'));
                end
                ...
                if ~isnumeric(featureLocations) || ~isreal(featureLocations) || issparse(featureLocations)
                    error(message('vision:bagOfFeaturespca:customLocationsMustBeRealNumeric'));
                end
                
                % cross validate number of features and locations
                if numFeatures ~= numLocations
                    error(message('vision:bagOfFeaturespca:customNumFeaturesNotEqNumLocations'));
                end
            end                       
        end
        ...
        function [imgSets, params] = parseInputs(varargin)                     
            ...
            parser = inputParser;
            parser.addRequired('imds', @(x)bagOfFeaturespca.checkImageSet(x,mfilename));
            
            parser.addParameter('VocabularySize',    d.VocabularySize,    @bagOfFeaturespca.checkVocabularySize);
            parser.addParameter('StrongestFeatures', d.StrongestFeatures, @bagOfFeaturespca.checkStrongestFeatures);
            parser.addParameter('PointSelection',    d.PointSelection,    @bagOfFeaturespca.checkAndAssignPointSelection);
            parser.addParameter('GridStep',          d.GridStep,          @bagOfFeaturespca.checkGridStep);
            parser.addParameter('BlockWidth',        d.BlockWidth,        @bagOfFeaturespca.checkBlockWidth);
            ...
            if bagOfFeaturespca.isImageSet(parser.Results.imds)                
            ...
            params.ExtractorOutputsLocations = bagOfFeaturespca.checkCustomExtractor(parser.Results.CustomExtractor);
            ...
            if wasSpecified('CustomExtractor')
                ...
                featureLength = bagOfFeaturespca.validateCustomExtractor(imgSets,params.CustomExtractor);
            ...
            if params.UsingCustomExtractor
                if wasPointSelectionSpecified || wasGridStepSpecified ...
                    ...
                    warning(message('vision:bagOfFeaturespca:customInvalidParamCombo'));
                end
            end
            ...
            [~, params.PointSelection] = ...
                bagOfFeaturespca.checkAndAssignPointSelection(parser.Results.PointSelection);
            
            % Warn about ignored options
            if strcmp(params.PointSelection, 'Detector')                
                if wasGridStepSpecified || wasBlockWidthSpecified
                   warning(message('vision:bagOfFeaturespca:paramsIgnored'));
                end                
            end
        end
        ...
        function params = parseEncodeInputs(varargin)            
            ...
            defaults = bagOfFeaturespca.getDefaultSettings;
            ...
            bagOfFeaturespca.validateEncodeInput(parser.Results.imds);
        ...
        function printImageSetDescription(printer,imgSets)
            cats = bagOfFeaturespca.getCategoryStrings(imgSets);
            ...
            for i = 1:numel(cats)
                printer.printMessage('vision:bagOfFeaturespca:imageSetDescription',i,cats{i});                
            end
    ...
    methods (Static)      
        function createExtractorTemplate()
            ...
            example = fullfile(toolboxdir('vision'),...
                'visionutilities','examplebagOfFeaturespcaExtractor.m');
            fid = fopen(example);
            ...
            contents = regexprep(editorDoc.Text,...
                'examplebagOfFeaturespcaExtractor', functionName,'once');
    ...
    methods (Hidden, Static)
        ...
        function this = loadobj(s)
            % reconstruct bagOfFeaturespca                       
            this = bagOfFeaturespca();    
            
            if all(isfield(s,{'CustomExtractor', 'Upright', 'ExtractorOutputsLocations'})) % added in R2015a                                                                 
                
                bagOfFeaturespca.checkCustomExtractorOnLoad(s.CustomExtractor);                               
